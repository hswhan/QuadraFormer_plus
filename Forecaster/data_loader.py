...
def build_revin_mask(all_columns, config):
    """
    True  = apply RevIN
    False = skip RevIN
    ...
    Conservative rule:
    - skip template_id
    - skip raw timestamp fields
    - skip explicitly configured categorical/hash columns
    - apply RevIN to continuous-valued columns, GNN embeddings, and diff features
    """
    raw_time_cols = {"YY", "MM", "DD", "HOUR", "MIN", "SEC", "HH"}

    # 可以在 config 里手动补充需要排除的列名
    extra_exclude = set(config.get("revin_exclude_cols", []))

    # 可以按前缀排除，比如 hash_, str_, cat_
    exclude_prefixes = tuple(config.get(
        "revin_exclude_prefixes",
        ["hash_", "str_", "string_", "cat_", "categorical_"]
    ))
    ...
    for col in all_columns:
        ...
        if c in raw_time_cols:
            skip = True

        if c in extra_exclude:
            skip = True

        if c.lower().startswith(exclude_prefixes):
            skip = True

        # 如果你的 feature_label 能明确标出 string/hash 参数，
        # 建议后续把对应 feature_i 加进 revin_exclude_cols。
    ...
    return np.array(mask, dtype=bool), excluded_cols
...
def load_data(config, scaler=None):
    ...
    if all_files_exist or config["processed"]=='True':
        ...
        config['all_columns'] = all_columns
        revin_mask_file = os.path.join(absolute_data_path, "revin_mask.npy")
        revin_excluded_file = os.path.join(absolute_data_path, "revin_excluded_cols.npy")

        if os.path.exists(revin_mask_file):
            revin_mask = np.load(revin_mask_file, allow_pickle=True).astype(bool)
        else:
            # 旧缓存没有 mask 时，重新按 columns 生成
            revin_mask, revin_excluded_cols = build_revin_mask(all_columns, config)
            np.save(revin_mask_file, revin_mask)
            np.save(revin_excluded_file, np.array(revin_excluded_cols, dtype=object))

        config["revin_mask"] = revin_mask

        if os.path.exists(revin_excluded_file):
            config["revin_excluded_cols"] = np.load(revin_excluded_file, allow_pickle=True).tolist()
        else:
            config["revin_excluded_cols"] = []

        # print(f"[RevIN] apply dims: {int(revin_mask.sum())}/{len(revin_mask)}")
        # print(f"[RevIN] excluded cols: {config['revin_excluded_cols']}")
    ...
    revin_mask, revin_excluded_cols = build_revin_mask(all_columns, config)
    np.save(os.path.join(absolute_data_path, "revin_mask.npy"), revin_mask)
    np.save(os.path.join(absolute_data_path, "revin_excluded_cols.npy"), np.array(revin_excluded_cols, dtype=object))

    config["revin_mask"] = revin_mask
    config["revin_excluded_cols"] = revin_excluded_cols

    print(f"[RevIN] apply dims: {int(revin_mask.sum())}/{len(revin_mask)}")
    print(f"[RevIN] excluded cols: {revin_excluded_cols}")
