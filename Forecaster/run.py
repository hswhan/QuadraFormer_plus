        ...
        for epoch in tqdm(range(self.config['epochs']), desc="Epoch Progress"):
            ...
            for batch in train_loader:
                ...
                if use_adaptive_tw:
                    ...
                    score_sql = (ema_sql + tw_eps).pow(tw_alpha)
                    score_res = (ema_res + tw_eps).pow(tw_alpha)
                    score_sum = score_sql + score_res + tw_eps

                    w_sql = 2.0 * score_sql / score_sum
                    w_res = 2.0 * score_res / score_sum
